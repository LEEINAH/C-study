...
namespace WindowsFormsApp4
{
	class Queen : Bee
	{
		...
		public string WorkTheNextShift()
		{
			...
			shiftNumber++;
			string report = "교대근무 보고 #" + shiftNumber + "\r\n";
			...
			for (int i = 0; i < workers.Length; i++)
			{
				...
				if (workers[i].DidYouFinish())
					report += "노동자 #" + (i + 1) + " 일을 끝냈다\r\n";
				if (String.IsNullOrEmpty(workers[i].CurrentJob))
					report += "노동자 #" + (i + 1) + " 작동하지 않습니다\r\n";
				else
					if (workers[i].ShiftsLeft > 0)
					report += "노동자 #" + (i + 1) + " 하고 있다 '" + workers[i].CurrentJob
						+ "' for " + workers[i].ShiftsLeft + " more shifts\r\n";
					else
						report += "노동자 #" + (i + 1) + " will be done with '"
							+ workers[i].CurrentJob + "' after this shift\r\n";
			}
			// 각 꿀벌들이 보고서를 한 줄 작성한다.
			// 그 후 여왕은 마지막 줄에 근무 시간 동안 소비한 총 꿀량을 작성한다.
			report += "교대 근무 동안 소비한 총 꿀: " + honeyConsumed + " units\r\n";
